...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        for j, a in enumerate(args):
            if isinstance(a, str):
                with contextlib.suppress(ValueError):
                    try:
                        args[j] = locals()[a] if a in locals() else ast.literal_eval(a) #?? a ????????????????????????????????
                    except:
                        args[j] = a
        ...
        if m in {Classify,Conv,ConvTranspose,GhostConv,Bottleneck,GhostBottleneck,SPP,SPPF,DWConv,Focus,
            BottleneckCSP,C1,C2,C2f,FrequencyFocusedDownSampling,FEDown,C3,
            C3TR,C3Ghost,nn.ConvTranspose2d,DWConvTranspose2d,C3x,C3k2,C2PSA,RepC3,SPDConv,
            ConvNormLayer
        }:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            ...
            if m in {BottleneckCSP,C1,C2,C2f,C3,C3TR,C3Ghost,C3x,C3k2,C2PSA,RepC3}:
