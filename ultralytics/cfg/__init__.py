...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    """
    Load and merge_results configuration data from a file or dictionary.
    ...
    if overrides:
        ...
        cfg = {**cfg, **overrides}  # merge_results cfg and overrides dicts (prefer overrides)
...
def merge_equals_args(args: List[str]) -> List[str]:
    ...
    for i, arg in enumerate(args):
        if arg == '=' and 0 < i < len(args) - 1:  # merge_results ['arg', '=', 'val']
            new_args[-1] += f'={args[i + 1]}'
            del args[i + 1]
        elif arg.endswith('=') and i < len(args) - 1 and '=' not in args[i + 1]:  # merge_results ['arg=', 'val']
            new_args.append(f'{arg}{args[i + 1]}')
            del args[i + 1]
        elif arg.startswith('=') and i > 0:  # merge_results ['arg', '=val']
            new_args[-1] += arg
        else:
            new_args.append(arg)
    return new_args
...
def entrypoint(debug=''):
    ...
    for a in merge_equals_args(args):  # merge_results spaces around '=' sign
        if a.startswith('--'):
