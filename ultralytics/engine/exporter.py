...
class Exporter:
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):
        ...
        if int8:
            ...
            # Force normalization layers to FP32 to prevent overflow in RT-DETR
            fp32_keywords = ["layernorm", "layer_norm", "instance_norm", "batch_norm", "normalize", "group_norm"]
            for i in range(network.num_layers):
                layer = network.get_layer(i)
                if any(k in layer.name.lower() for k in fp32_keywords):
                    layer.precision = trt.float32
                    for j in range(layer.num_outputs):
                        layer.set_output_type(j, trt.float32)
            LOGGER.info(f"{prefix} forced normalization layers to FP32 precision")
